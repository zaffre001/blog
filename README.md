...
| `web/index.html` | 쉘 페이지 + 플레이어 임베드 + 디버그 패널 |
...
open http://localhost:8123/web/player/blog.html   # 블로그 본편 (첫 클릭으로 시작)
open http://localhost:8123/web/                   # 디버그 패널 (글리프·디더링 확인)
```

블로그 본편은 **익스포트된 공식 플레이어 페이지**(`web/player/blog.html`)다 —
PICO-8 웹 빌드는 키보드·마우스를 공식 템플릿의 JS 글루(`pico8_buttons`/`pico8_mouse`)로
읽기 때문에, 우리 스크립트(`bridge.js`)를 그 페이지에 주입하는 구조를 쓴다
(`tools/export.sh`가 자동 주입). 첫 화면의 ▶ 클릭은 브라우저 오디오 정책 때문에 필요.
